...
class CategoryProduct extends Controller
{
    ...
    public function list_category_product(){
        ...
        $manager = view('admin.list_category_product')->with('list_category_product',$list_category_product);
        return view('admin_layout')->with('admin.list_category_product',$manager);
    }

    public function save_category_product(Categoryvalidation $request){
        $this->AuthLogin();
        $data = array();
    ...
    public function active_category_product(Request $request,$category_product_id){
        $this->AuthLogin();
        DB::table('tbl_category_product')->where('category_id', $category_product_id)->update(['category_status'=>2]);
        $request->session()->put('message', 'Hiển Thị Danh Mục');
        return redirect('list-category-product');
    }

    public function unactive_category_product(Request $request, $category_product_id){
        $this->AuthLogin();
        DB::table('tbl_category_product')->where('category_id', $category_product_id)->update(['category_status'=>1]);
        $request->session()->put('message', 'Không Hiển Thị Danh Mục');
        return redirect('list-category-product');
    }
    ...
    public function update_category_product(Categoryvalidation $request,$category_product_id){
        $this->AuthLogin();
        $data = array();
        $data['category_name'] = $request->category_product_name;
        $data['category_desc'] = $request->category_product_desc;
    ...
    public function delete_category_product(Request $request,$category_product_id){
        ...
        $request->session()->put('message', 'Xoá Danh Mục thành công');
        return redirect('list-category-product');
    }
    ...
    public function show_category_home($category_id){
        ...
        return view('pages.category.show_category')->with('category',$cate_product)->with('brand',$brand_product)->with('list_product',$list_product);
    }
}
